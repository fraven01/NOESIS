...
# --- Definition der initialen Daten ---
...
def create_initial_data(apps, schema_editor):
    """Erstellt alle notwendigen initialen Daten."""
    # Modelle aus der Migration holen
    ...
    Anlage2GlobalPhrase = apps.get_model("core", "Anlage2GlobalPhrase")
    ...
    if not all_users.exists():
        print(
            "WARNUNG: Keine Benutzer gefunden. Kacheln werden erstellt, aber niemandem zugewiesen."
        )
    ...
    for status_data in INITIAL_PROJECT_STATUSES:
        status, created = ProjectStatus.objects.update_or_create(
            key=status_data["key"], defaults=status_data
        )
        if created:
            print(f"  -> Status '{status.name}' wurde erstellt.")

    # 3. LLM-Rollen
    print("\n[3] Verarbeite LLM-Rollen...")
    for role_data in INITIAL_LLM_ROLES:
        role, created = LLMRole.objects.update_or_create(
            name=role_data["name"], defaults=role_data
        )
        if created:
            print(f"  -> LLM-Rolle '{role.name}' wurde erstellt.")

    # 4. Anlage 1 Fragen und Varianten
    print("\n[4] Verarbeite Anlage 1 Fragen...")
    for q_data in INITIAL_ANLAGE1_QUESTIONS:
        question, q_created = Anlage1Question.objects.update_or_create(
            num=q_data["num"],
            defaults={
                "text": q_data["text"],
        ...
        if q_created:
            print(f"  -> Frage {question.num} wurde erstellt.")

        variants_in_db = []
        for variant_text in q_data.get("variants", []):
            variant, _ = Anlage1QuestionVariant.objects.update_or_create(
                question=question, text=variant_text
            )
            variants_in_db.append(variant.id)
        Anlage1QuestionVariant.objects.filter(question=question).exclude(
            id__in=variants_in_db
        ).delete()

    # 5. Anlage 2 Funktionen und Unterfragen
    print("\n[5] Verarbeite Anlage 2 Funktionen...")
    for func_data in INITIAL_ANLAGE2_FUNCTIONS:
        func, func_created = Anlage2Function.objects.update_or_create(
            name=func_data["name"]
        )
        if func_created:
            print(f"  -> Funktion '{func.name}' wurde erstellt.")
        subquestions_in_db = []
        for sub_question_text in func_data.get("subquestions", []):
            sub_q, _ = Anlage2SubQuestion.objects.update_or_create(
                funktion=func, frage_text=sub_question_text
            )
            subquestions_in_db.append(sub_q.id)
        Anlage2SubQuestion.objects.filter(funktion=func).exclude(
            id__in=subquestions_in_db
        ).delete()

    # 6. Anlage 2 Konfiguration
    print("\n[6] Verarbeite Anlage 2 Konfiguration...")
    config, config_created = Anlage2Config.objects.update_or_create(
        singleton_enforcer=True, defaults={"parser_mode": "auto"}
    )
    if config_created:
        print("  -> Anlage2Config-Objekt wurde erstellt.")

    # Globale Phrasen
    for phrase_data in INITIAL_ANLAGE2_CONFIG["global_phrases"]:
        phrase, created = Anlage2GlobalPhrase.objects.update_or_create(
            phrase_type=phrase_data["phrase_type"],
            defaults={"phrase_text": phrase_data["phrase_text"]},
        )
        if created:
            print(f"    -> Globale Phrase '{phrase.phrase_type}' wurde erstellt.")

    # Alias-Überschriften
    for heading_data in INITIAL_ANLAGE2_CONFIG["alias_headings"]:
        heading, created = Anlage2ColumnHeading.objects.update_or_create(
            config=config,
        ...
        if created:
            print(
                f"    -> Alias-Überschrift für '{heading.field_name}' wurde erstellt."
            )
...
class Migration(migrations.Migration):
    dependencies = [
        ("core", "0001_initial"),
        ("auth", "0001_initial"),  # Stellt sicher, dass das User-Modell verfügbar ist
    ]
